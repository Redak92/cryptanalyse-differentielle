...
\begin{frame}{Phase de détection : implémentation pratique}
    
    \begin{block}{Changement de paradigme}
        \begin{itemize}
            \item \textbf{Approche naïve :} Itération exhaustive sur les $2^n$ valeurs de $\alpha$.
            \item \textbf{Par collisions :} Itération ciblée sur un échantillon de $M$ messages.
        \end{itemize}
    \end{block}

    \vspace{0.4cm}
    \textbf{Mécanique de l'algorithme en 4 étapes :}
    \vspace{0.2cm}

    \begin{enumerate}
        \item \textbf{Génération :} Tirage de $M$ messages et calcul des signatures $g_\gamma(x)$.
        \item \textbf{Hachage :} Insertion en mémoire pour identifier les doublons $g_\gamma(x) = g_\gamma(x')$.
        \item \textbf{Déduction :} Pour chaque collision, calcul de la différence $\alpha = x \oplus x'$.
        \item \textbf{Comptage :} Incrémentation du score du candidat $\alpha$.
    \end{enumerate}
...
\begin{frame}{Le goulot d'étranglement spatial}
