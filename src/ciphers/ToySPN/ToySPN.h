//
// Created by alexandre on 30/01/2026.
//
#include <vector>
#include "../../utils/Types.h"
#include "../../interfaces/ICipher.h"

#ifndef CRYPTANALYSE_DIFFERENTIELLE_TOYSPN_H
#define CRYPTANALYSE_DIFFERENTIELLE_TOYSPN_H

/* ToySpn :
 * 
 * Architecture : 
 *      - Blocksize of 16 bits 
 *      - Sbox of size 8
 *      - Pbox of size 16
 *      - Key of size 16
 * 
 * Round function F(K, R):
 *      Permutates key through Pbox rounds_number times
 */

class ToySPN : public ICipher{
    public : 
        ToySPN(Key init_key, int init_rounds_number) ;

        Block encrypt(Block message) const ;
        Block decrypt(Block message) const ;
        int getBlockSize() const ;

    private : 
        constexpr static uint32_t BLOCK_SIZE = 16 ;        
        constexpr static uint32_t SBOX_SIZE = 256 ; 
        constexpr static uint32_t PBOX_SIZE = 16 ;        
        
        uint32_t rounds_number ;
        std::vector<Block> round_keys ;
        Key key ;    
        
        // AES's 8bit Sbox, initialised with the initialize_aes_sbox function from the Rijndael S-box wikipedia page
        constexpr static uint8_t SBOX_ARRAY[SBOX_SIZE] = {
                 
        };

        constexpr static uint8_t INVERSE_SBOX_ARRAY[SBOX_SIZE] = { 
                 
        } ; 
        
        // 16 bit Pbox
        constexpr static uint8_t PBOX_ARRAY[PBOX_SIZE] = {5,9,0,13,7,2,11,14,1,4,12,8,3,15,6,10}; 
        constexpr static uint8_t INVERSE_PBOX_ARRAY[PBOX_SIZE] = {2,8,5,12,9,0,14,4,11,1,15,6,10,3,7,13}  ;

        // Functions to use the Sbox
        static uint8_t substitute(uint8_t message) ; 
        static uint8_t inv_substitute(uint8_t message) ; 
        
        // Functions to use the Pbox
        static Block permutate(Block message) ;
        static Block inv_permutate(Block message) ;

        // Generates all round keys as a vector, based on rounds_number
        static std::vector<Block> key_schedule(Key key,int rounds_nb) ; 
};

#endif //CRYPTANALYSE_DIFFERENTIELLE_TOYSPN_H